...
const googleClientKey = 'cookie-google-client-id';
let googleClientId = sessionStorage.getItem(googleClientKey) || document.querySelector('meta[name="google-client-id"]')?.content.trim() || '';
const authState = {
  mode: 'cookie',
  accessToken: '',
  cloudFiles: JSON.parse(sessionStorage.getItem('cookie-google-files') || '{}'),
};
...
function initGoogleAuth() {
  ...
  tokenClient = window.google.accounts.oauth2.initTokenClient({
    ...
    callback: async (response) => {
      if (response.error) {
        setStorageNotice(`Google 登入失敗：${response.error}`);
        return;
      }
      authState.mode = 'google';
      authState.accessToken = response.access_token;
      storageMode.textContent = 'Google 雲端同步';
      googleLoginBtn.querySelector('span').textContent = '已登入 Google';
...
function handleGoogleLogin() {
  if (!googleClientId) {
    ...
    setStorageNotice('Google 登入會使用 Google 官方 OAuth 視窗，不會在本頁輸入密碼。請先貼上 OAuth Client ID；未設定前仍以 Cookie／瀏覽器暫存模式使用。');
    return;
  }
  if (!tokenClient) initGoogleAuth();
  if (!tokenClient) {
    setStorageNotice('Google 登入元件尚未載入完成，請稍後再試。');
    return;
  }
  googleConfig.hidden = true;
  tokenClient.requestAccessToken({ prompt: authState.accessToken ? '' : 'consent' });
}

function saveGoogleClientId() {
  const value = googleClientInput.value.trim();
  if (!value) {
    setStorageNotice('請貼上 Google OAuth Client ID 後再啟用登入。');
    return;
  }
  googleClientId = value;
  sessionStorage.setItem(googleClientKey, value);
  tokenClient = null;
  googleConfig.hidden = true;
  setStorageNotice('Google OAuth Client ID 已暫存於本分頁，請再次點選「登入 Google」開啟官方驗證視窗。');
}
...
function scheduleLiveDetection() {
  ...
  renderDetection({ message: '正在準備搜尋 Crossref、OpenAlex、Semantic Scholar、DataCite、Europe PMC、arXiv、Google Books、Open Library、Library of Congress，並保留 Airiti、Google Scholar、JSTOR、WorldCat 等導向路徑。' }, 'loading');
...
async function saveToLibrary(item, options = {}) {
  try {
    ...
    if (authState.mode === 'google' && authState.accessToken) await syncGoogleLibrary();
    renderLibrary(libraryState);
...
function renderLibrary(summary) {
  ...
  if (authState.mode === 'google') {
    libraryUpdated.textContent = summary.updatedAt ? `Google 雲端已同步：${new Date(summary.updatedAt).toLocaleString('zh-TW')}` : 'Google 雲端同步模式';
    preparedFilePath.textContent = authState.cloudFiles.markdownName || 'Google Drive://Cookie Prepared Bibliography.md';
    renderLibrarySelector();
    renderLibraryHistory();
    renderLibraryEditor();
    return;
...
async function deleteEditorSelection() {
  ...
  const ok = window.confirm(`確定要從個人文獻資料庫刪除 ${chosen.length} 筆文獻嗎？這會同步更新 Cookie 暫存${authState.mode === 'google' ? '與 Google Drive 檔案' : ''}。`);
  if (!ok) return;
...
async function deleteLibraryItem(item, options = {}) {
  ...
  persistSelectedRefs();
  sessionStorage.setItem(cookieKey, JSON.stringify(libraryState));
  if (!options.skipSync && authState.mode === 'google' && authState.accessToken) await syncGoogleLibrary();
  renderLibrary(libraryState);
  render();
  if (selectedRefKeys.size) buildSelectedBibliography();
  else bibliographyOutput.textContent = '尚未選取文獻。';
...
async function syncGoogleLibrary() {
  const folder = await ensureGoogleDriveFolder('全功能文獻管理代理人');
  const payload = {
    updatedAt: libraryState.updatedAt || new Date().toISOString(),
    count: libraryState.items.length,
    items: libraryState.items,
  };
  const markdown = preparedBibliographyMarkdown(libraryState.items, payload.updatedAt);
  const jsonFile = await upsertGoogleDriveFile('Research Library.json', 'application/json', `${JSON.stringify(payload, null, 2)}\n`, authState.cloudFiles.jsonId, folder.id);
  const markdownFile = await upsertGoogleDriveFile('Prepared Bibliography.md', 'text/markdown', markdown, authState.cloudFiles.markdownId, folder.id);
  authState.cloudFiles = {
    folderId: folder.id,
    folderName: `Google Drive://${folder.name}`,
    jsonId: jsonFile.id,
    markdownId: markdownFile.id,
    markdownName: `Google Drive://${folder.name}/${markdownFile.name}`,
  };
  sessionStorage.setItem('cookie-google-files', JSON.stringify(authState.cloudFiles));
}

async function ensureGoogleDriveFolder(name) {
  if (authState.cloudFiles.folderId) return { id: authState.cloudFiles.folderId, name };
  const existing = await findGoogleDriveFolder(name);
  ...
  if (!response.ok) throw new Error(`Google Drive 資料夾建立失敗：${response.status}`);
  return response.json();
}

async function findGoogleDriveFolder(name) {
  const q = encodeURIComponent(`name='${name.replace(/'/g, "\\'")}' and mimeType='application/vnd.google-apps.folder' and trashed=false`);
  const response = await fetch(`https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name)&pageSize=1`, {
...
async function upsertGoogleDriveFile(name, mimeType, content, knownId, parentId = '') {
  const metadata = { name, mimeType };
  if (parentId && !knownId) metadata.parents = [parentId];
  const boundary = `cookie_${Date.now()}`;
  const body = [
    `--${boundary}`,
    'Content-Type: application/json; charset=UTF-8',
    '',
  ...
  const id = knownId || await findGoogleDriveFile(name, parentId);
  const url = id
  ...
  if (!response.ok) throw new Error(`Google Drive 同步失敗：${response.status}`);
  return response.json();
}

async function findGoogleDriveFile(name, parentId = '') {
  const parentQuery = parentId ? ` and '${parentId}' in parents` : '';
  const q = encodeURIComponent(`name='${name.replace(/'/g, "\\'")}'${parentQuery} and trashed=false`);
  const response = await fetch(`https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name)&pageSize=1`, {
    headers: { authorization: `Bearer ${authState.accessToken}` },
  });
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  googleClientInput.value = googleClientId;
  initGoogleAuth();
  loadLibrary();
  render();
});
